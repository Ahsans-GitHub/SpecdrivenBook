#!/usr/bin/env python3
"""
Simple test script to verify the API integration between frontend and backend
"""
...
async def test_specific_endpoints():
            ...
            try:
                async with session.get(f"{base_url}{endpoint}") as resp:
                    if resp.status in [200, 405]:  # 405 is OK for endpoints that don't accept GET
                        print(f"   [SUCCESS] {description} ({endpoint}): {resp.status}")
                        success_count += 1
                    else:
    ...
    print(f"   [SUCCESS] {success_count}/{len(endpoints_to_test)} endpoints accessible")
...
async def main():
    ...
    success1 = await test_api_connection()
