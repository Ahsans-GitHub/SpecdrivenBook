...
if ($Help) {
    Write-Host "Usage: ./create-new-feature.ps1 [-Json] [-ShortName <name>] [-Number N] [-FeatureDescriptionFilePath <path>] [<feature description>]"
    Write-Host ""
    Write-Host "Options:"
    Write-Host "  -Json                         Output in JSON format"
    Write-Host "  -ShortName <name>             Provide a custom short name (2-4 words) for the branch"
    Write-Host "  -Number N                     Specify branch number manually (overrides auto-detection)"
    Write-Host "  -FeatureDescriptionFilePath <path>  Path to a file containing the feature description"
    Write-Host "  -Help                         Show this help message"
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  ./create-new-feature.ps1 'Add user authentication system' -ShortName 'user-auth'"
    Write-Host "  ./create-new-feature.ps1 -FeatureDescriptionFilePath './description.txt'"
    exit 0
}

# Determine feature description
$featureDesc = ''
if (-not [string]::IsNullOrEmpty($FeatureDescriptionFilePath)) {
    if (Test-Path $FeatureDescriptionFilePath) {
        $featureDesc = Get-Content $FeatureDescriptionFilePath -Raw
    } else {
        Write-Error "Error: Feature description file not found at '$FeatureDescriptionFilePath'"
        exit 1
    }
